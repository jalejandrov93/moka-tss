...
#
# Path resolution for the Mascota dashboard, correct whether it runs from
...
def base_dir() -> Path:
    ...
    # library/mascota/paths.py -> library -> mascota -> repo root
...
def user_data_dir() -> Path:
    ...
    resolve inside sys._MEIPASS.
      - Windows: %APPDATA%\\Mascota (falls back to ~/AppData/Roaming/Mascota
        if APPDATA is unset).
      - Everywhere else: the XDG base directory spec, i.e. $XDG_DATA_HOME
        /mascota, falling back to ~/.local/share/mascota.
    """

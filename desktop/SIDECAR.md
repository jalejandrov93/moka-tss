...
| Consumed endpoint | `GET http://127.0.0.1:<port>/api/status` every 2000 ms (`desktop/src/main.ts:3-4`, `STATUS_URL` + `POLL_INTERVAL_MS`, `setInterval(fetchStatus, ...)`). Served by `library/moka_tss/webconfig.py:322-324` from `MokaApp._get_status` (`library/moka_tss/app.py`), which currently returns 4 keys (`tick`, `running`, `agenthub_available`, `codexbar_available`). Aligning it with the 7-key `status_snapshot()` contract is pending (MD-2.2). |
| Status payload (target) | `{ tick, running, agenthub_available, codexbar_available, has_system, has_snapshot, has_state }` — see `desktop/src/types.ts` and `status_snapshot()`. Until MD-2.2 lands, the live `/api/status` omits the three `has_*` keys and the frontend must treat them as missing. Frontend renders `Error fetching status` on non-2xx or network failure. |

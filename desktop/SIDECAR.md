...
| Status payload | `{ tick, running, agenthub_available, codexbar_available, has_system, has_snapshot, has_state, mood, system, screen, transmission }` (11 keys) — see `desktop/src/types.ts` and `status_snapshot()`. Live `/api/status` serves all 11 keys (`mood` is `string | null`, added in MD-2.3; `system` is `{ cpu, ram, gpu }` and `screen` is `{ present, simulate, brightness }`, added in MD-2.8a; `transmission` is the screen transmission stats from `Screen.stats_snapshot()`, added in MD-4.2). Frontend renders `Error fetching status` on non-2xx or network failure. |

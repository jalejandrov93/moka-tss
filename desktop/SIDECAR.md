...
| Status payload | `{ tick, running, agenthub_available, codexbar_available, has_system, has_snapshot, has_state, mood }` (8 keys) — see `desktop/src/types.ts` and `status_snapshot()`. Live `/api/status` serves all 8 keys (`mood` is `string | null`, added in MD-2.3). Frontend renders `Error fetching status` on non-2xx or network failure. |

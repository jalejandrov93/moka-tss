...
| T6 | **hecha y mergeada** | `mascot.py` + 14 PNGs (64 KB) + `CREDITS.md` + **20 tests**. Commits `1351b81` + merge `3f96422`. Primera entrega tenía 4 de 6 moods con la misma silueta recoloreada; rechazada tras mirar una hoja de contacto. La segunda da 6 siluetas distintas, verificadas **en escala de grises**. Incluye un test que mide la diferencia de siluetas entre cada par y falla bajo umbral. | n/a (RDD off) |
| T7 | **en corrección** | `screen.py` (180) + `render.py` (324) + 42 tests, pero el autor admitió **no haber hecho TDD estricto**: escribió la implementación y después movió los archivos para fabricar un RED. La auditoría adversarial encontró 2 defectos reales (ver HD-6) y 5 tests tautológicos. Protocolo verificado limpio. | n/a |
| T11 | en curso (deleg) | Panel web local, worktree `t11-webconfig`. | — |
| T12 | en curso (deleg) | Rutas para build congelado, worktree `t12-exe`. | — |
| T9, T10 | pendiente | — | — |
...
| HD-6 | auditoría de T7 | Dos defectos en el compositor, ambos **reproducidos por el padre**: (1) un `usedPercent` string revienta `render()` con `TypeError: '>' not supported between 'int' and 'str'` y congela la pantalla — misma clase de bug que tenía T5; (2) `brightness_absolute` valida con `assert`, y **los `assert` desaparecen en un build optimizado**: con `python3 -O`, `brightness_absolute(110)` devuelve `-25`, que enmascarado a byte llega al panel como `231`, casi apagado. El segundo importa justo porque el objetivo es un `.exe` de PyInstaller. | `render({'providers':[{'id':'c','windows':[{'usedPercent':'50'},{'usedPercent':0}]}]}, None, sys)` y `python3 -O -c "brightness_absolute(110)"` | Tratar todo lo que no sea número real como dato ausente; acotar o rechazar el porcentaje en vez de confiar en `assert`. **Nunca usar `assert` para validar entradas en código que se congela.** |
| HD-7 | auditoría de T7 | Cinco tests tautológicos: recalculan la expresión de producción dentro del test, así que seguirían pasando si el código estuviera mal de la misma forma. Parecen cobertura sin serlo. Son el subproducto típico de escribir tests después de la implementación. | `test_field_layout_matches_wire_protocol`, `test_landscape_frame_fields`, `test_set_brightness_writes_inverted_level`, `test_amber_threshold_is_inclusive`, `test_attentive_at_amber_threshold` | Reescribirlos contra literales calculados a mano, no contra una recomputación. |
| HD-8 | delegación | **Bug en agent-hub v2.1.0**: `delegate` falla siempre con `delegateTool(...).then is not a function`, incluso en modo lectura con el modelo más barato y una tarea trivial. El resto de los tools anda. Funcionó al principio de la sesión y dejó de andar, lo que sugiere que el servidor se reinició y levantó el código con el defecto. | `src/tools/jobs.mjs:36` declara `export function delegateTool(...)` **síncrona**, que devuelve `{jobId, status, errorKind}`; `src/index.mjs:254` la llama como `delegateTool({...}).then((res) => ...)`. Un objeto plano no tiene `.then`. Runtime y repo de desarrollo son idénticos. | Sacar el `.then()` y usar el objeto devuelto directo, o hacer `delegateTool` `async`. Es repo del usuario (`jalejandrov93/agent-hub`), no se tocó sin permiso. Mientras tanto se delega con subagentes Claude, que es lo que el mapa marca para `implementation-with-repo-rules`. |
